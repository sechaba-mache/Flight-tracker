...
import { storage } from "./src/models/stored";
...
let historyHeading: HTMLElement | null = document.querySelector(".historyHeading");
    ...
    if (flights.length != 0) {
        if (flightBoxes.length > 0) {
            ...
            Array.from(flights).map((currentFlight, index) =>
                updateFlightInfo(currentFlight, flightBoxes[index] as HTMLElement)
            );
        } else {
        ...
        if (flightBoxes.length != 0)
            Array.from(flightBoxes).map((box: HTMLElement) => box.remove());
...
function openHistory(): void {
    ...
    const stored: storage[] = JSON.parse(localStorage.getItem("flights") ?? "[]");

    Array.from(stored).map((element: storage) => createHistory(element));
}

function createHistory(element: storage): void {
    const divContainingFlights: HTMLElement = document.querySelector(".content")!;
...
function closeHistory(): void {
    ...
    newSub = allFlights$.subscribe((res: IFlightResponse | ofError) => {
